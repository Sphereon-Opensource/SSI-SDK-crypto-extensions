...
export const validateX509CertificateChain = async ({
    ...
    return await validateX509CertificateChainImpl({reversed: false, chain: pemOrDerChain.reverse(), trustAnchors, verificationTime, opts})
}
const validateX509CertificateChainImpl = async ({
    ...
    for (let i = 0; i < chainLength; i++) {
        ...
        if (i > 0) {
            if (cert.x509Certificate.issuer !== chain[i -1].x509Certificate.subject) {
                if (!reversed && !disallowReversedChain) {
                    return  await validateX509CertificateChainImpl({reversed: true, chain: pemOrDerChain.reverse(), opts, verificationTime, trustAnchors})
                }
                return {
                        error: true,
                        critical: true,
        ...
        const result = await cert.x509Certificate.verify({date: verificationTime, publicKey: prevCert?.x509Certificate?.publicKey }, crypto)
        if (!result) {
...
export type  ParsedCertificate = {publicKeyInfo: SubjectPublicKeyInfo, publicKeyJwk: JWK, publicKeyRaw: Uint8Array, publicKeyAlgorithm: Algorithm, certificateInfo: CertificateInfo, certificate: Certificate, x509Certificate: X509Certificate}
...
export const parseCertificate = async (rawCert: string | Uint8Array): Promise<ParsedCertificate> => {
    ...
    return { publicKeyAlgorithm, publicKeyInfo, publicKeyJwk, publicKeyRaw, certificateInfo, certificate, x509Certificate}
}
